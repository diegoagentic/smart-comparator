...
export default function OCRTracking({ onLogout, onNavigate, onConvertDocument }: OCRTrackingProps) {
    ...
    const filteredDocs = documents.filter(d => {
        ...
        const matchesDeprecatedFilter = activeTab !== 'deprecated' || deprecatedFilter === 'All' || d.deprecatedReason === deprecatedFilter || (deprecatedFilter === 'Canceled' && d.deprecatedReason === 'Cancelled')
        
        let matchesTime = true;
        if (activeTab === 'deprecated' && timeFilter !== 'All Time') {
            const dateStr = d.date.toLowerCase();
            let days = 0;
            if (dateStr.includes('today') || dateStr.includes('hour')) days = 0;
            else if (dateStr.includes('yesterday')) days = 1;
            else if (dateStr.includes('days ago')) days = parseInt(dateStr.match(/\d+/)?.[0] || '0');
            else if (dateStr.includes('week ago')) days = 7;
            else if (dateStr.includes('weeks ago')) days = parseInt(dateStr.match(/\d+/)?.[0] || '0') * 7;
            else if (dateStr.includes('month ago')) days = 30;
            else if (dateStr.includes('months ago')) days = parseInt(dateStr.match(/\d+/)?.[0] || '0') * 30;

            if (timeFilter === 'Last 7 days') matchesTime = days <= 7;
            else if (timeFilter === 'Last 30 days') matchesTime = days <= 30;
            else if (timeFilter === 'Last 90 days') matchesTime = days <= 90;
        }

        return matchesSearch && matchesTab && matchesDeprecatedFilter && matchesTime
    })
                                ...
                                <div className="flex gap-1 p-1 rounded-lg w-fit overflow-x-auto max-w-full">
                                    {[
                                        ...
                                        <button
                                            ...
                                            className={`px-3 py-1.5 text-sm rounded-md transition-all flex items-center gap-2 outline-none whitespace-nowrap ${activeTab === tab.id
                                                    ? 'bg-[#e2f373] text-zinc-900 shadow-sm font-semibold'
                                                    : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100 dark:hover:bg-zinc-800 font-medium'
                                                }`}
                                        >
                                            <span className="leading-none">{tab.label}</span>
                                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold transition-colors ${activeTab === tab.id ? 'bg-black/10 text-zinc-900' : 'text-zinc-400'
                                                }`}>{tab.count}</span>
                                        </button>
                                    ))}
                                </div>
                        ...
                        {activeTab === 'deprecated' ? (
                            <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                <div className="flex items-center justify-between pb-2">
                                    <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
                                        {['All', 'Superseded', 'Canceled', 'Duplicate', 'Vendor Correction', 'Merged', 'Failed Processing', 'Manually Archived', 'Obsolete', 'Other'].map(f => (
                                            <button
                                                ...
                                                className={`px-3 py-1.5 text-xs font-medium rounded-full whitespace-nowrap transition-all ${deprecatedFilter === f ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'text-muted-foreground hover:bg-zinc-100 hover:text-zinc-900'}`}
                                            >
                                                {f}
                                            </button>
                                        ))}
                                    ...
                                    <div className="relative shrink-0">
                                        <button
                                            ...
                                            className="flex items-center gap-2 bg-[#FAFAFA] hover:bg-zinc-100 p-1.5 rounded-lg border border-[#D0D4D8] transition-colors"
                                        >
                                            <span className="text-[11px] font-bold text-zinc-900 px-1">{timeFilter}</span>
                                            <ChevronDown className={`size-3 text-zinc-500 transition-transform ${isTimeDropdownOpen ? 'rotate-180' : ''}`} />
                                        </button>

                                        {isTimeDropdownOpen && (
                                            <>
                                                <div className="fixed inset-0 z-10" onClick={() => setIsTimeDropdownOpen(false)} />
                                                <div className="absolute right-0 mt-2 w-40 bg-white border border-border rounded-xl shadow-xl py-1.5 z-20 animate-in fade-in zoom-in-95 duration-200">
                                                    {['Last 7 days', 'Last 30 days', 'Last 90 days', 'All Time'].map(t => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-4 py-2 text-[12px] transition-colors ${timeFilter === t
                                ...
                                {filteredDocs.length > 0 ? (
                                            ...
                                            <div key={doc.id} className="bg-card border border-border/50 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all group flex flex-col gap-4 relative overflow-hidden">
                                                <div className="flex items-start justify-between">
                                                    <div className="flex items-center gap-3">
                                                        <div className={`h-10 w-10 rounded-xl flex items-center justify-center transition-colors ${doc.deprecatedReason === 'Superseded' ? 'bg-blue-50 text-blue-600' :
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            const statusMap: Record<string, string> = {
                                                                ...
                                                                'Awaiting Expert': 'inconsistencies',
                                                                'In Progress': 'in_progress',
                                                                'Needs Attention': 'capturing',
                                                                'Ingesting': 'identified'
                                                            };
                                                            setDocuments(prev => prev.map(d => d.id === doc.id ? { ...d, status: (doc.wasStatus && statusMap[doc.wasStatus]) || 'inconsistencies' } : d))
                                                        }}
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${doc.deprecatedReason === 'Superseded' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30' :
                                                ...
                                                <div className="mt-auto pt-4 border-t border-border/50 flex items-center justify-between">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <div className={`px-2 py-1 rounded-lg flex items-center gap-1 text-[11px] font-bold ${(doc.confidence || 0) > 90 ? 'bg-green-100 text-green-700 dark:bg-green-900/30' :
                                    ...
                                    <div className="border-2 border-dashed border-border rounded-2xl p-12 text-center bg-zinc-50/50">
                                        <p className="text-sm font-medium text-zinc-400">No documents found matching this filter</p>
                                    </div>
                                )}
                            </div>
                        ) : (
                                                ...
                                                <div key={column.id} className="min-w-[280px] max-w-[320px] flex-1 space-y-3 snap-start">
                                                    ...
                                                    <div className="space-y-3">
                                                        {docs.map(doc => (
                                                            <div
                                                                key={doc.id}
                                                                className={`group relative bg-card dark:bg-zinc-800 rounded-2xl border transition-all duration-200 overflow-hidden flex flex-col ${selectedDoc === doc.id
                                                            ...
                                                            >
                                                                <div className="p-4">
                                                                    <div className="flex items-center justify-between mb-3">
                                                                        <div className="flex items-center gap-2">
                                                                            <div className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold shadow-sm ring-2 ring-white dark:ring-zinc-900 ${doc.status === 'processed' ? 'bg-gradient-to-br from-green-500 to-green-700 text-white' :
                                                                    ...
                                                                    <div className="pt-3 border-t border-border flex items-center justify-between">
                                                                        <div className="flex items-center gap-2">
                                                                            {doc.confidence ? (
                                                                                <span title="Confidence Score" className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${doc.confidence > 90 ? 'bg-green-50 text-green-700 dark:bg-green-500/15 dark:text-green-300 ring-1 ring-inset ring-green-600/20 dark:ring-green-400/30' :
                                                                                ...
                                                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg uppercase tracking-wider ${doc.type === 'Acknowledgment' ? 'bg-blue-50 text-blue-700 ring-1 ring-blue-700/10' :
                                                                                        doc.type === 'Purchase Order' ? 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-700/10' :
                                                                                            doc.type === 'Invoice' ? 'bg-purple-50 text-purple-700 ring-1 ring-purple-700/10' :
                                                                                                'bg-zinc-100 text-zinc-600'
                                                                                    }`}>
                                                                                    {doc.type === 'Acknowledgment' ? 'ACK' : doc.type === 'Purchase Order' ? 'PO' : doc.type === 'Invoice' ? 'INV' : doc.type}
                                                                                </span>
                                                                            )}
                                                                        </div>
                                ...
                                {viewMode === 'list' && (
                                    <div className="overflow-hidden rounded-xl border border-border">
                                        <table className="w-full">
                                            ...
                                            <tbody>
                                                {filteredDocs.map(doc => (
                                                    <tr key={doc.id} className="border-b border-border hover:bg-zinc-100/60 transition-colors">
                                                        ...
                                                        <td className="px-4 py-3">
                                                            <span className={`text-xs font-semibold px-2 py-1 rounded-md ${doc.status === 'processed' ? 'bg-success-light text-success' :
                                                            ...
                                                            <div className="flex items-center justify-end gap-1">
                                                                ...
                                                                {doc.status === 'deprecated' ? (
                                                                    <button
                                                                        ...
                                                                        className="p-1.5 rounded-lg hover:bg-zinc-100 text-muted-foreground hover:text-zinc-900"
